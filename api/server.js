...
// ── Importación única de datos (TEMPORAL — quitar tras la migración) ─────────
// Solo superadmin. Recibe { businesses?, files: [{businessId, name, content}] }
// y los escribe en el volumen de datos. Sirve para replicar el estado local
// en producción una sola vez. Protegido por rol superadmin.
app.post('/api/admin/import', authenticate, async (req, res) => {
  ...
  try {
    ...
    for (const f of (files || [])) {
      // Validaciones anti path-traversal
